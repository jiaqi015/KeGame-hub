...
import type { Case, ClosedDealRecord, CustomerCaseRuntime, CustomerRuntimeState, Opportunity } from '../src/selling-houses/domain/models.js';
import {
...
// ---------------------------------------------------------------------------
// C receipt dimension alignment checks (customer-runtime)
// ---------------------------------------------------------------------------
...
const allRegistries = [
  ...
  ...CUSTOMER_RUNTIME_STATE_FIELD_OWNERSHIP_ENTRIES.map((e) => ({ ...e, registry: 'CustomerRuntimeState' })),
  ...CUSTOMER_CASE_RUNTIME_FIELD_OWNERSHIP_ENTRIES.map((e) => ({ ...e, registry: 'CustomerCaseRuntime' })),
];
...
const totalFields = LEGACY_CASE_FIELD_OWNERSHIP_ENTRIES.length
  ...
  + CUSTOMER_RUNTIME_STATE_FIELD_OWNERSHIP_ENTRIES.length
  + CUSTOMER_CASE_RUNTIME_FIELD_OWNERSHIP_ENTRIES.length;
...
// Verify no CustomerRuntime fields use 'evaluation-mirror' as canonical owner
for (const entry of CUSTOMER_RUNTIME_STATE_FIELD_OWNERSHIP_ENTRIES) {
  assert.notEqual(
    entry.canonicalOwner as string,
    'evaluation-mirror',
    `CustomerRuntimeState.${entry.field} should not be evaluation-mirror`,
  );
}
