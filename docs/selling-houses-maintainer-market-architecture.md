...
`lost_to_rival` 现在应该被理解成“维护窗口被别人抓住”，不是普通热度下滑。

触发时同时需要两类条件：

- 我方露出经营空档，例如窗口极短、业主关系断档、信任崩塌、没有有效客户漏斗、暗线客户被其他经纪人牵走
- 对方确实有机会，例如商圈竞争压力高、同组价格差明显、我方价格偏离市场、存在 broker shadow lead

为了避免玩家刚维护过仍被系统随机判死，同一套房会记录 `lastRivalThreatDay`，竞品失守判定有冷却；同时“最近维护过且信任不低”的房源会被保护，除非客户漏斗已经明显空掉且竞品价格/压力优势很强。

这个设计的业务含义是：

- 竞品可以每天压热度、抢客户、制造价格锚
- 但真正“被截走”必须来自可解释的维护破绽
- 标准以上压力局可以经常出现截走，新手局不应该把截走做成常态
...
- 验证平均分从 warmup 到 extreme 大体单调下降
- 验证坏收尾、竞品房源、商圈事件密度随难度抬升
...
- 旧存档兼容稳定
- 六难度 20 seed 验证通过，不能只看 5 seed 小样本
- 新机制带来可解释的过程型失守
