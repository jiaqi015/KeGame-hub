import { Clock, RotateCw, GitCompare, XCircle } from 'lucide-react';
import type { OpenDayAnalysisSnapshotSummary } from '../../domain/openDay.types.ts';
      ...
      <div className="open-day-history-list">
        {snapshots.length > 0 ? (
          snapshots.map((item) => (
            <div
              key={item.id}
              className={`open-day-history-card ${activeSnapshotId === item.id ? 'is-active' : ''} ${baselineSnapshotId === item.id ? 'is-baseline' : ''}`}
            >
              <div className="open-day-history-card__body" onClick={() => onReplay(item.id)}>
                <div className="open-day-history-card__main">
                  ...
                  {baselineSnapshotId === item.id && <span className="open-day-baseline-tag">当前基准</span>}
                </div>
              ...
              <div className="open-day-history-card__actions">
                {baselineSnapshotId === item.id ? (
                  <button 
                    className="open-day-baseline-btn is-active" 
                    onClick={(e) => { e.stopPropagation(); onClearBaseline(); }}
                    title="取消基准方案"
                  >
                    <XCircle size={14} />
                  </button>
                  ...
                  <button 
                    className="open-day-baseline-btn" 
                    onClick={(e) => { e.stopPropagation(); onSetBaseline(item.id); }}
                    title="设为对比基准"
                  >
                    <GitCompare size={14} />
                  </button>
