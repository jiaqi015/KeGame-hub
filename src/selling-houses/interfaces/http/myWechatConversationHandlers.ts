...
export async function handleMyWechatConversationTurn(input: unknown): Promise<MyWechatConversationTurnHandlerResult> {
  ...
  if (!scene) {
    ...
    return {
      ...
      body: {
        ok: false,
        proposal: buildFallbackConversationEffectProposal(fallbackScene),
        source: 'fallback',
        error: '微信对话上下文不可用。',
      },
    };
  ...
  try {
    const proposal = parseConversationEffectProposalPayload(result.result, scene);
    return {
      status: 200,
      body: {
        ok: true,
...
function buildWechatConversationTurnPrompt(scene: ConversationSceneInputPack) {
  return [
    ...
    '5. 语气像真实微信：recipientReply 是对方看到玩家回复后的自然反应，不要像系统结算文案。',
    '6. 不说“系统/AI/模型/评分/内部变量”。不承诺“一定成交/保证结果”。',
    '',
    'delta 建议范围：trustDelta、patienceDelta、urgencyDelta、priceFlexibilityDelta 通常在 -4 到 5；非常明确且合理才更高。',
