...
function buildOwnerMood(role: CaseRole, ownerArchetype: OwnerArchetype, prototype: HousePrototype) {
  if (role === 'anchor') {
    return `希望你给出清晰方案，别浪费 ${prototype.community} 这次机会`;
  }
  if (role === 'fragile') {
    return ownerArchetype.id === 'trial-balloon' ? '再拖几天就想核销' : '推进很紧，动作慢一点就焦虑';
  }
  if (role === 'traffic') {
  ...
  if (blueprint.eventArcId === 'relationship_recovery') {
    if (fragile) {
      ...
      events.push({
        ...
        actor: '业主朋友圈',
        title: '同类盘回暖',
        message: `${targetCase?.ownerName || '业主'}看到同类房源有成交进展，愿意多听你一点推进说明。`,
        tone: 'success',
    ...
    if (anchor) {
      ...
      events.push({
        ...
        actor: '业主家庭',
        title: '配合节奏确认',
        message: `${targetCase?.ownerName || '业主'}确认了接下来两周的安排，愿意多给你一点操作空间。`,
        tone: 'success',
