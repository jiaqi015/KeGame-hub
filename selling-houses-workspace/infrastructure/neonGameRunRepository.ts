...
function resolveMatterRecommendedActionCodes(
  ...
  const fallbackGroups = [
    { match: ['推进', '稳住', '信任', '业主'], codes: ['weekly-feedback', 'first-visit'] },
...
function buildMatterResponseText(
  ...
  if (!actionNames.length) {
    return `先处理：${matter.title}。`;
  }

  return `先处理：${matter.title}。可做动作：${actionNames.join(' / ')}。`;
}
...
function buildResolvedCaseMatterSummary(
  ...
  if (caseItem.status === 'withdrawn') {
    return `${caseItem.title} 已核销，当前经营动作应视为取消。`;
  }

  if (caseItem.status === 'lost_to_rival') {
    return `${caseItem.title} 已在他处成交，这条线已经结束。`;
  }
...
function buildResolvedCaseMatterTitle(
  ...
  if (caseItem.status === 'withdrawn') return `${caseItem.title} 已核销`;
  if (caseItem.status === 'lost_to_rival') return `${caseItem.title} 他处成交`;
...
function resolveClosedMatterStatus(
  ...
  if (caseItem.status === 'withdrawn') {
    return {
      ...
      resolutionSummary: '房源已核销，后续动作取消。',
    };
  }
  ...
  return {
    ...
    resolutionSummary: '房源已在他处成交，这件事现在做不成了。',
  };
}
...
function resolveEventMatterId(
  ...
  if (haystack.includes('竞品') || haystack.includes('流失') || haystack.includes('推进')) {
    const riskMatter = preferredRows.find((row) => {
      const text = normalizeSearchText(`${row.title} ${row.summary}`);
      return text.includes('推进') || text.includes('稳住') || text.includes('失控') || text.includes('流失');
    });
    if (riskMatter) {
          ...
          const placeholders = priorities.map((entry, index) => {
            ...
            values.push(
              ...
              caseId
                ...
                : '如果不尽快处理，这条线会继续失速。',
              String(entry.detail || '把当前最影响结果的事项先推进掉。'),
              100 - index * 10,
            );
